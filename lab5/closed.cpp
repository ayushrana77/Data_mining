...
void find_closed(vector<set<int>> &prev,vector<set<int>> &curr,map<set<int>,int>&allsup_count)
{
	...
	for(auto item: curr)
	{
		...
		for(auto i: item)
		{
			...
			for(auto j: prev)
			{
				...
				if((temp == j) && (allsup_count[j] == allsup_count[item]))
				{
					isexit = true;
					break;
				}
...
void apri_algo(vector<set<int>>&tran,int min_sup)
{
	...
	while(!freqitems.empty())
	{
		...
		find_closed(maxfreq,freqitems,allsup_count);
